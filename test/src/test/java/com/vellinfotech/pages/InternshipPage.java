...
public class InternshipPage {
    ...
    public InternshipPage(WebDriver driver, WebDriverWait wait) {
        this.driver = driver;
        this.wait = wait;
    ...
    private By internshipLinks = By.cssSelector("a[href*='internship'], a[href*='intern']");
    ...
    public boolean homepageHasInternshipNav() {
        for (WebElement link : driver.findElements(navLinks)) {
            ...
            if (text.contains("internship") || href.contains("internship")) {
                return true;
            }
        }
        return false;
    ...
    public void clickInternshipNav() {
        for (WebElement link : driver.findElements(navLinks)) {
            String text = safeText(link).toLowerCase();
            String href = safeAttr(link, "href").toLowerCase();
        ...
        throw new NoSuchElementException("Internship navigation link not found.");
    }
    ...
    public List<WebElement> getVisibleInternshipLinks() {
        List<WebElement> out = new ArrayList<>();
        for (WebElement el : driver.findElements(internshipLinks)) {
            String href = safeAttr(el, "href");
            String text = safeText(el);
            if (isDisplayed(el) && (!href.isEmpty() || !text.isEmpty())) {
                out.add(el);
            }
        }
        return out;
    ...
    public List<String> getVisibleInternshipLinkNames() {
        LinkedHashSet<String> names = new LinkedHashSet<>();
        for (WebElement el : getVisibleInternshipLinks()) {
    ...
    public boolean hasInternshipRelatedText(String... keywords) {
    ...
    public boolean clickFirstInternshipLink() {
        List<WebElement> links = getVisibleInternshipLinks();
    ...
    public int fillInternshipForm(String name, String email, String phone, String message) {
        int count = 0;
    ...
    public void printFullPageAudit() {
        System.out.println("========== INTERNSHIP PAGE AUDIT ==========");
        ...
        System.out.println("\n--- INTERNSHIP LINKS ---");
        for (String c : getVisibleInternshipLinkNames()) System.out.println(" - " + c);
    }
